...
export interface StarterRepository {
  ...
  listFeedings(starterId: string, limit?: number, offset?: number): Promise<Feeding[]>;
  listObservedFeedings(starterId: string, limit: number, excludedId?: string): Promise<Feeding[]>;
  ...
  expirePastReminders(nowMs: number): Promise<void>;
  listReminderFeedings(nowMs: number): Promise<Feeding[]>;
  listPhotoPaths(starterId?: string): Promise<string[]>;
  listRemindersWithNotificationIds(): Promise<Reminder[]>;
