import type { EstimateFactor, FlourType, PeakEstimate, PeakObservation } from "./models";
...
const flourTimeMultipliers: Record<FlourType, number> = { white: 1, whole_wheat: 0.9, rye: 0.82, blend: 0.95, other: 1 };

export function calculateFeedingRatio(input: Pick<FeedingInput, "starterGrams" | "flourGrams" | "waterGrams">) {
  validateAmounts(input);
  return { starter: 1, flour: input.flourGrams / input.starterGrams, water: input.waterGrams / input.starterGrams };
}
...
export function estimatePeak(input: FeedingInput, observations: readonly PeakObservation[] = []): PeakEstimate {
  ...
  const calibratedRatio = clamp(flourToStarter, 0.25, 20);
  const calibratedHydration = clamp(hydration, 50, 200);
  if (calibratedRatio !== flourToStarter || calibratedHydration !== hydration) outsideCalibration = true;
  let midpointHours = Math.max(2, 6 + 1.7 * Math.log2(calibratedRatio));
  ...
  return {
    ...
    earliestAtMs: toTimestamp(Math.max(1, midpointHours - halfWidthHours)),
    midpointAtMs: toTimestamp(midpointHours),
    latestAtMs: toTimestamp(Math.min(48, midpointHours + halfWidthHours)),
...
function derivePersonalization(observations: readonly PeakObservation[]) {
  const residuals = observations
    .filter((item) => Number.isFinite(item.predictedMidpointHours) && Number.isFinite(item.observedElapsedHours) && item.predictedMidpointHours >= 2 && item.predictedMidpointHours <= 36 && item.observedElapsedHours >= 2 && item.observedElapsedHours <= 36 && Math.abs(item.observedElapsedHours - item.predictedMidpointHours) <= 12)
    .slice(-12)
    .map((item) => item.observedElapsedHours - item.predictedMidpointHours);
  if (residuals.length < MIN_PERSONALIZATION_OBSERVATIONS) return { applied: false as const, offsetHours: 0, validObservationCount: residuals.length, reason: "not_enough_observations" as const };
  const offsetHours = median(residuals);
  const deviation = median(residuals.map((residual) => Math.abs(residual - offsetHours)));
  if (deviation > 3) return { applied: false as const, offsetHours: 0, validObservationCount: residuals.length, reason: "observations_too_variable" as const };
  return { applied: true as const, offsetHours: clamp(offsetHours, -4, 4), validObservationCount: residuals.length, reason: "applied" as const };
}
...
function validateInput(input: FeedingInput) {
  ...
  validateAmounts(input);
  if (input.temperatureC !== undefined && (!Number.isFinite(input.temperatureC) || input.temperatureC < -50 || input.temperatureC > 80)) throw new RangeError("temperatureC must be between -50 and 80");
}
